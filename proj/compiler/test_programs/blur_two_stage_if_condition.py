...
def two_stage_blur(input_img):
    ...
    for r in range(input_img.shape[0]-8):
        for c in range(input_img.shape[1] - 1):
            if c > 0:
                temp_img[r, c] = (input_img[r, c] + input_img[r + 1, c] + input_img[r + 2, c]) / 3.0
            else:
                temp_img[r, c] = (input_img[r, c] + input_img[r + 1, c] + input_img[r + 2, c]) / 3.0
   
        temp_img[r, input_img.shape[1] - 1] = (input_img[r, input_img.shape[1] - 1] + input_img[r + 1, input_img.shape[1] - 1] + input_img[r + 2, input_img.shape[1] - 1]) / 3.0
    
    #second stage blur
    #transform(LoopFusion('two_stage_blur', 'output_img'))
    for r in range(input_img.shape[0]-8):
