...
def interpolate(input_img):
    ...
    w = input_img.shape[0]
    h = input_img.shape[1]
    downsampled = np.zeros([levels, w, h, 4])
    downx = np.zeros([levels, w, h, 4])
    interpolated = np.zeros([levels, w, h, 4])
    upsampled = np.zeros([levels, w, h, 4])
    upsampledx = np.zeros([levels, w, h, 4])
