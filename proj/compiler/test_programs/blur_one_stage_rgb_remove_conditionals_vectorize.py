...
def test(n=None,
         input_img_rgb = util.image_filename('rgba.png'),
 ...
 #   gaussian_blur(np.zeros((100,100)))
    gaussian_blur(np.zeros((100,100,4),'float32'), np.zeros((100,100,4),'float32'))
    ans1 = util.test_image_pipeline_filename(gaussian_blur, (input_img_rgb,), n, name='blur_one_stage.gaussian_blur rgb', use_output_img=True, dtype='float32')
...
def test_large_image():
    test(input_img_rgb  = util.image_filename('large_temple_rgb.png'),
         input_img_gray = util.image_filename('large_temple_gray.png'))
