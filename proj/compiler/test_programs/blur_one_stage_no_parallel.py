...
#transform(TypeSpecialize(({'_nonvar_range(input_img.shape[0])':('int',),'_nonvar_range(input_img.shape[1])':('int',),'c':'int','input_img':'numpy.ndarray[numpy.float64_t, ndim=3](shape=(405, 640, 4),shape_list=[])','kernel_accum':'numpy.ndarray[numpy.float64_t, ndim=1](shape=(4,),shape_list=[])','kernel_norm':'double','output_img':'numpy.ndarray[numpy.float64_t, ndim=3](shape=(405, 640, 4),shape_list=[])','r':'int'},),False))     # applied to original line 13
...
#transform(LoopRemoveConditionals())     # applied to original line 30
    for r in range(input_img.shape[0]):
        for c in range(input_img.shape[1]):
            # center
#transform(VectorizeInnermost())     # applied to original line 33
...
#transform(VectorizeInnermost())     # applied to original line 76
            output_img[r, c] = kernel_accum / kernel_norm
    
    return output_img

input_img_rgb = util.image_filename('rgb_small2.png')
